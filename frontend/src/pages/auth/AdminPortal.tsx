...
function getStatusStyle(status: string) {
  ...
  if (status === "Finalizado") {
    return "bg-green-500/20 text-emerald-700 border-emerald-200"
  }
...
export function AdminPortal() {
  ...
  if (!loggedEmployee) {
    return (
      <div className="flex min-h-screen items-center justify-center ls-page-shell p-4">
        <Card className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white shadow-[0_25px_80px_rgba(15,23,42,0.45)]">
          <CardContent className="space-y-6 p-8">
            <div className="space-y-2 text-center">
              ...
              <h1 className="text-3xl font-bold text-slate-950">Acesse sua conta</h1>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="rounded-2xl border-slate-200 bg-slate-50 px-4 py-3 text-slate-950"
                />
              </div>
              ...
              <Button
                className="w-full rounded-[1.75rem] bg-blue-600 text-slate-950 shadow-lg shadow-sky-500/20 hover:bg-blue-700"
  ...
  if (successState.isVisible) {
    return (
      <div className="flex min-h-screen items-center justify-center ls-page-shell p-4">
        <Card className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white shadow-[0_25px_80px_rgba(15,23,42,0.5)]">
          <CardContent className="space-y-8 p-12 text-center">
            ...
            <div className="space-y-3">
              <h1 className="text-3xl font-bold text-slate-950">Chamado enviado!</h1>
              <p className="text-slate-500">
                Seu chamado foi criado com sucesso. Agora você pode acompanhar respostas por aqui.
              </p>
            </div>

            <div className="space-y-2 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-6">
              <div>
                <p className="text-xs uppercase tracking-wider text-slate-500">ID do Chamado</p>
                <p className="text-xl font-semibold text-emerald-700">#{successState.ticketId}</p>
              </div>
              <div className="border-t border-slate-200 pt-4">
                <p className="text-xs uppercase tracking-wider text-slate-500">Solicitante</p>
                <p className="text-lg text-slate-950">{successState.employeeName}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 pt-4 sm:grid-cols-3">
              <Button
                variant="outline"
                className="rounded-[1.5rem] border-slate-300 text-slate-700 hover:bg-slate-100/50"
                onClick={handleExit}
              >
                Sair
              </Button>
              <Button
                variant="outline"
                className="rounded-[1.5rem] border-blue-200 text-blue-700 hover:bg-blue-700/10"
                onClick={handleGoToMyTickets}
              >
                Acompanhar
              </Button>
              <Button
                className="rounded-[1.5rem] bg-emerald-600 text-slate-950 shadow-lg shadow-green-500/20 hover:bg-emerald-700"
  ...
  return (
    <div className="min-h-screen ls-page-shell p-4 text-slate-950">
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-7xl flex-col gap-6">
        <header className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-[0_25px_80px_rgba(15,23,42,0.35)]">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-slate-500">
                Setor vinculado: <strong className="text-slate-800">{loggedEmployee.sector?.name}</strong>. Seu setor é puxado do cadastro e será usado em todos os chamados.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Button
                variant="outline"
                className="rounded-2xl border-slate-300 text-slate-700 hover:bg-slate-100/60"
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center gap-3">
                <UserCircle2 className="text-blue-700" />
                <div>
                  <p className="text-sm font-semibold text-slate-950">{loggedEmployee.name}</p>
                  <p className="text-xs text-slate-500">Usuário: {loggedEmployee.username || "sem usuário"}</p>
                </div>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-1">
              <button
        ...
        {activeTab === "new" ? (
          <Card className="rounded-[2rem] border border-slate-200 bg-white shadow-[0_25px_80px_rgba(15,23,42,0.35)]">
            <CardContent className="space-y-6 p-8">
              <div className="space-y-3">
                <div className="inline-flex rounded-full bg-blue-50 px-4 py-2 text-sm font-medium text-blue-700">
                  Abrir chamado interno
                </div>
                <h2 className="text-3xl font-bold text-slate-950">Preencha os dados</h2>
                <p className="text-slate-500">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-sm text-slate-500">Solicitante</label>
                  <Input
                    value={loggedEmployee.name}
                    disabled
                    className="rounded-2xl border-slate-200 bg-slate-50 px-4 py-3 text-slate-500"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-sm text-slate-500">Categoria</label>
                  <select
                    value={category}
                    onChange={(e) => setCategory(e.target.value)}
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-800 shadow-sm outline-none focus:border-sky-500"
                  >
                ...
                <div className="space-y-2">
                  <label className="text-sm text-slate-500">Origem</label>
                  <select
                    value={origin}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  className="min-h-[180px] rounded-2xl border-slate-200 bg-slate-50 px-4 py-3 text-slate-950"
                />
              </div>

              <Button
                className="w-full rounded-[1.75rem] bg-blue-600 py-6 text-slate-950 shadow-lg shadow-sky-500/20 hover:bg-blue-700 disabled:opacity-50"
            ...
            <Card className="rounded-[2rem] border border-slate-200 bg-white shadow-[0_25px_80px_rgba(15,23,42,0.35)]">
              <CardContent className="flex h-full flex-col p-5">
                <div className="mb-4 flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-slate-950">Meus chamados</h2>
                ...
                <div className="space-y-3 overflow-y-auto pr-1">
                  {myTickets.map((ticket) => (
                    <button
                      ...
                      className={`w-full rounded-2xl border p-4 text-left transition hover:border-blue-300 hover:bg-slate-50 ${
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold text-slate-950">#{ticket.id} - {ticket.category}</p>
                      ...
                      <p className="mt-3 text-xs text-slate-500">{ticket.createdAt}</p>
                    </button>
                  ))}
                ...
                {selectedTicket ? (
                  <>
                    <div className="border-b border-slate-200 p-6">
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                          <p className="text-xs uppercase tracking-[0.22em] text-blue-700">Conversa do chamado</p>
                        ...
                        return (
                          <div key={message.id} className={`flex ${isEmployee ? "justify-end" : "justify-start"}`}>
                            <div className={`max-w-[82%] rounded-3xl border p-4 ${
                              isEmployee
                                ? "border-blue-200 bg-sky-500/15 text-blue-950"
                      ...
                      {selectedTicket.status === "Finalizado" ? (
                        ...
                        <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
                          <Textarea
                            ...
                            className="min-h-[96px] rounded-2xl border-slate-200 bg-slate-50 text-slate-950"
                          />
                          <Button
                            className="rounded-2xl bg-blue-600 px-6 text-slate-950 hover:bg-blue-700 disabled:opacity-50"
                  ...
                  <div className="flex h-full min-h-[640px] items-center justify-center p-8 text-center">
                    <div className="max-w-md">
                      ...
                      <h2 className="mt-4 text-2xl font-bold text-slate-950">Selecione um chamado</h2>
