...
const PAGE_TITLES: Record<string, { label: string; sub: string }> = {
  "/home":      { label: "Início", sub: "Menu do painel técnico" },
...
export function Header({ notificationCount = 0 }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-[color:var(--hairline)] bg-white/85 px-3 py-3 backdrop-blur-md sm:px-4 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 sm:gap-4">
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-[color:var(--muted-foreground)]">{page.sub}</p>
        </div>

        <div className="flex items-center gap-2 sm:gap-3">
          <button
            onClick={handleNotificationClick}
            className={`relative rounded-lg border border-[color:var(--hairline)] bg-white p-2 text-[#0B3D29] shadow-[var(--shadow-xs)] transition hover:border-[#00A859]/35 hover:bg-[color:var(--accent)] sm:p-2.5 ${notificationCount > 0 ? "cursor-pointer" : "cursor-default"}`}
            aria-label={`Notificações${notificationCount > 0 ? ` (${notificationCount})` : ""}`}
          >
            <Bell size={18} strokeWidth={2} />
            {notificationCount > 0 && (
              <span className="ls-num absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#E11D48] px-0.5 text-[9px] font-bold text-white shadow-sm sm:h-[18px] sm:min-w-[18px] sm:px-1 sm:text-[10px]">
                {notificationCount > 99 ? "99+" : notificationCount}
              </span>
            )}
          </button>

          <div className="hidden items-center gap-2.5 rounded-lg border border-[color:var(--hairline)] bg-white px-3 py-2 shadow-[var(--shadow-xs)] sm:flex">
